...
CREATE DEFINER=`alex`@`localhost` PROCEDURE `addcommandprogram`
( IN u VARCHAR(15), IN c VARCHAR(15), IN idpa INTEGER, IN n INTEGER)
...
CREATE DEFINER=`alex`@`localhost` PROCEDURE `removecommandprogram`
( IN u VARCHAR(15), IN c VARCHAR(15), IN idpa INTEGER, IN n INTEGER)
...
SELECT `USERS`.`USERNAME` AS `USERNAME` , `FUNCTIONS`.`FUNCTION` AS `FUNCTION` , count( 0 ) AS `TOTAL` , sum( if( (
`HISTORYACCESS`.`ERROR` =0
), 1, 0 ) ) AS `SUCCESS` , sum( if( (
`HISTORYACCESS`.`ERROR` <>0
), 1, 0 ) ) AS `ERROR` , sum( if( (
`HISTORYACCESS`.`ERROR` =2
), 1, 0 ) ) AS `PASSWORD` , sum( if( (
`HISTORYACCESS`.`ERROR` =4
), 1, 0 ) ) AS `INTEGRITY`
FROM (
(
(
`USERS`
INNER JOIN `HISTORYACCESS` ON ( (
`USERS`.`IDUSER` = `HISTORYACCESS`.`IDUSER`
) )
)
LEFT JOIN `FUNCTIONS` ON ( (
`HISTORYACCESS`.`FUNCT` = `FUNCTIONS`.`FUNCT`
) )
)
JOIN `ERRORS` ON ( (
`ERRORS`.`ERRORCODE` = `HISTORYACCESS`.`ERROR`
) )
)
GROUP BY `USERS`.`USERNAME` , `FUNCTIONS`.`FUNCTION`
ORDER BY count( 0 ) DESC
) AS T



(SELECT USERS.IDUSER, USERNAME, FUNCT, COUNT(*)  FROM USERS LEFT JOIN HISTORYACCESS USING (IDUSER)
GROUP BY USERS.IDUSER, USERNAME, FUNCT)
UNION
(SELECT USERS.IDUSER, USERNAME, FUNCT, COUNT(*)  FROM USERS RIGHT JOIN HISTORYACCESS USING (IDUSER)
GROUP BY USERS.IDUSER, USERNAME, FUNCT)
ORDER BY USERNAME
...
CREATE TRIGGER testnoadmin BEFORE INSERT ON HISTORYACCESS
  FOR EACH ROW BEGIN
IF (NEW.IDUSER = 0) THEN 
    INSERT INTO `HISTORYACCESS`(`IDHISTORY`, `IDUSER`, `IDHOUSE`, `ERROR`, `FUNCT`, `DATESTAMP`) VALUES 
(NEW.`IDHISTORY`, NULL ,NEW.`IDHOUSE` ,NEW.`ERROR` ,NEW.`FUNCT`, NEW.`DATESTAMP` );
ELSE
INSERT INTO `HISTORYACCESS`(`IDHISTORY`, `IDUSER`, `IDHOUSE`, `ERROR`, `FUNCT`, `DATESTAMP`) VALUES 
(NEW.`IDHISTORY`, NEW.`IDUSER` ,NEW.`IDHOUSE` ,NEW.`ERROR` ,NEW.`FUNCT`, NEW.`DATESTAMP` );
END IF;
  END
//
...
DELIMITER //
CREATE VIEW houseRoomServiceActionVIEW AS
SELECT HOUSES.IDHOUSE, HOUSENAME, ROOMS.IDROOM, ROOMNAME, SERVICES.IDSERVICE, SERVICENAME, ACTIONS.IDACTION
	FROM HOUSES
	JOIN ROOMS 		USING ( IDHOUSE )
	JOIN SERVICES	USING ( IDROOM )
	JOIN ACTIONS	USING ( IDSERVICE )
...
end_proc:begin
	...
	INSERT INTO HISTORYACCESS
						(IDHISTORY, IDUSER, IDHOUSE, ERROR, FUNCT, DATESTAMP        )
				VALUES  (     NULL,    idu,   idh,  IF(err = 62, 0, err),  30, CURRENT_TIMESTAMP);
...
CREATE OR REPLACE ALGORITHM=UNDEFINED DEFINER=`alex`@`localhost` SQL SECURITY DEFINER VIEW `loginVIEW` AS select `USERS`.`IDUSER` AS `IDUSER`,`USERS`.`USERNAME` AS `USERNAME`,`USERS`.`PASSWORD` AS `PASSWORD`,`HOUSES`.`IDHOUSE` AS `IDHOUSE`,`HOUSES`.`HOUSENAME` AS `HOUSENAME`,IMAGES.URL AS URL,`ROOMS`.`IDROOM` AS `IDROOM`,`ROOMS`.`ROOMNAME` AS `ROOMNAME`,`SERVICES`.`IDSERVICE` AS `IDSERVICE`,`SERVICES`.`SERVICENAME` AS `SERVICENAME`,SERVICES.`STATUS` AS `STATUS`, SERVICES.SERVICEINTERFACE AS SERVICEINTERFACE,`ACTIONS`.`IDACTION` AS `IDACTION`,`ACTIONS`.`ACTIONNAME` AS `ACTIONNAME`,`ACCESSHOUSE`.`ACCESSNUMBER` AS `ACCESSNUMBER`,`PERMISSIONS`.`PERMISSIONNUMBER` AS `PERMISSIONNUMBER`,`DEVICES`.`IDDEVICE` AS `IDDEVICE`,`DEVICES`.`IPADDRESS` AS `IPADDRESS`,`HOUSES`.`CITY` AS `CITY`,`HOUSES`.`COUNTRY` AS `COUNTRY` 
from ((((((((`USERS` left join `ACCESSHOUSE` on((`USERS`.`IDUSER` = `ACCESSHOUSE`.`IDUSER`))) 
left join `HOUSES` on((`ACCESSHOUSE`.`IDHOUSE` = `HOUSES`.`IDHOUSE`))) 
LEFT JOIN IMAGES ON (HOUSES.IDIMAGE = IMAGES.IDIMAGE) )
left join `ROOMS` on((`ACCESSHOUSE`.`IDHOUSE` = `ROOMS`.`IDHOUSE`))) 
left join `SERVICES` on((`ROOMS`.`IDROOM` = `SERVICES`.`IDROOM`))) 
left join `DEVICES` on((`SERVICES`.`IDDEVICE` = `DEVICES`.`IDDEVICE`))) 
left join `ACTIONS` on((`SERVICES`.`IDSERVICE` = `ACTIONS`.`IDSERVICE`))) 
left join `PERMISSIONS` on(((`PERMISSIONS`.`IDUSER` = `USERS`.`IDUSER`) and (`PERMISSIONS`.`IDSERVICE` = `SERVICES`.`IDSERVICE`)))) where 1 order by `USERS`.`USERNAME`,`HOUSES`.`HOUSENAME`,`ROOMS`.`ROOMNAME`,`SERVICES`.`SERVICENAME`,`ACTIONS`.`ACTIONNAME` desc;

CREATE OR REPLACE ALGORITHM=UNDEFINED DEFINER=`alex`@`localhost` SQL SECURITY DEFINER VIEW `commandVIEW` AS SELECT USERS.IDUSER AS IDUSER, USERNAME, COMMANDS.IDCOMMAND AS IDCOMMAND,  COMMANDNAME, COMMAND_PROGRAM.POS AS POS, PROGRAMACTIONS.IDPROGRAM AS IDPROGRAM, HOUSES.IDHOUSE AS IDHOUSE, HOUSENAME, ROOMS.IDROOM AS IDROOM, ROOMNAME, SERVICES.IDSERVICE AS IDSERVICE, SERVICENAME, ACTIONS.IDACTION AS IDACTION, ACTIONNAME, PROGRAMACTIONS.DATA AS DATA
FROM USERS
LEFT JOIN COMMANDS USING (IDUSER)
LEFT JOIN COMMAND_PROGRAM USING (IDCOMMAND)
LEFT JOIN PROGRAMACTIONS USING (IDPROGRAM)
LEFT JOIN ACTIONS USING (IDACTION)
LEFT JOIN SERVICES USING (IDSERVICE)
LEFT JOIN ROOMS USING (IDROOM)
LEFT JOIN HOUSES USING (IDHOUSE)
ORDER BY USERNAME , COMMANDNAME,POS ASC, HOUSENAME, ROOMNAME, SERVICENAME, ACTIONNAME;

CREATE OR REPLACE ALGORITHM=UNDEFINED DEFINER=`alex`@`localhost` SQL SECURITY DEFINER VIEW `taskVIEW` AS (SELECT USERS.IDUSER AS IDUSER, USERNAME, TASKS.IDTASK AS IDTASK,  TASKNAME, TASKS.DESCRIPTION AS DESCRIPTION, PROGRAMACTIONS.STARTTIME AS STARTTIME, PROGRAMACTIONS.IDPROGRAM AS IDPROGRAM, HOUSES.IDHOUSE AS IDHOUSE, HOUSENAME, ROOMS.IDROOM AS IDROOM, ROOMNAME, SERVICES.IDSERVICE AS IDSERVICE, SERVICENAME, ACTIONS.IDACTION AS IDACTION, ACTIONNAME, PROGRAMACTIONS.DATA AS DATA
FROM USERS
LEFT JOIN TASKS USING (IDUSER)
LEFT JOIN TASKPROGRAM USING (IDTASK)
LEFT JOIN PROGRAMACTIONS USING (IDPROGRAM)
LEFT JOIN ACTIONS USING (IDACTION)
LEFT JOIN SERVICES USING (IDSERVICE)
LEFT JOIN ROOMS USING (IDROOM)
LEFT JOIN HOUSES USING (IDHOUSE)
ORDER BY USERNAME , TASKNAME,STARTTIME ASC, HOUSENAME, ROOMNAME, SERVICENAME, ACTIONNAME)
UNION
(SELECT USERS.IDUSER AS IDUSER, USERNAME, TASKS.IDTASK AS IDTASK,  TASKNAME, TASKS.DESCRIPTION AS DESCRIPTION, PROGRAMACTIONS.STARTTIME AS STARTTIME, PROGRAMACTIONS.IDPROGRAM AS IDPROGRAM, HOUSES.IDHOUSE AS IDHOUSE, HOUSENAME, ROOMS.IDROOM AS IDROOM, ROOMNAME, SERVICES.IDSERVICE AS IDSERVICE, SERVICENAME, ACTIONS.IDACTION AS IDACTION, ACTIONNAME, PROGRAMACTIONS.DATA AS DATA
FROM TASKS
INNER JOIN USERS USING (IDUSER)
RIGHT JOIN PROGRAMACTIONS USING (IDUSER)
LEFT JOIN TASKPROGRAM USING (IDTASK, IDPROGRAM)
LEFT JOIN ACTIONS USING (IDACTION)
LEFT JOIN SERVICES USING (IDSERVICE)
LEFT JOIN ROOMS USING (IDROOM)
LEFT JOIN HOUSES USING (IDHOUSE)
...
CREATE VIEW singleProgramVIEW AS 
SELECT USERS.IDUSER AS IDUSER, USERNAME , PROGRAMACTIONS.IDPROGRAM, IDACTION, DATA, STARTTIME, HOUSES.IDHOUSE AS IDHOUSE, HOUSENAME, ROOMS.IDROOM AS IDROOM, ROOMNAME, SERVICES.IDSERVICE AS IDSERVICE,  SERVICENAME, ACTIONS.IDACTION AS IDACTION, ACTIONNAME 
FROM USERS 
JOIN PROGRAMACTIONS USING (IDUSER)
LEFT JOIN TASKPROGRAM USING (IDPROGRAM)  
LEFT JOIN COMMAND_PROGRAM USING (IDPROGRAM)
LEFT JOIN ACTIONS USING (IDACTION)
LEFT JOIN SERVICES USING (IDSERVICE)
LEFT JOIN ROOMS USING (IDROOM)
LEFT JOIN HOUSES USING (IDHOUSE)
WHERE TASKPROGRAM.IDPROGRAM IS NULL AND COMMAND_PROGRAM.IDPROGRAM IS NULL
ORDER BY USERNAME, IDPROGRAM;
...
CREATE TRIGGER auto-generateService AFTER INSERT ON SERVICES
FOR EACH ROW
BEGIN
SELECT COUNT(*), IDSERVICE, VERSION INTO num, idd, v
FROM SERVICES
WHERE DEVICENAME = d AND IDUSER IS NULL AND IPADDRESS IS NULL AND SERIAL IS NULL;
				
	DECLARE cur CURSOR FOR SELECT  `SERVICENAME`, `SERVICEINTERFACE`, `STATUS`, `ENGLISH`, `SPANISH`
						FROM SERVICES
						WHERE SERVICENAME = idd;
...
END;
  ...
  SELECT `USERS`.`IDUSER` AS `IDUSER` , `USERS`.`USERNAME` AS `USERNAME` , `DEVICES`.`IDDEVICE` AS `IDDEVICE` , `SERVICES`.`IDSERVICE` AS `IDSERVICE` , `SERVICES`.`SERVICENAME` AS `SERVICENAME` , `SERVICES`.`IDROOM` AS `IDROOM`, ROOMNAME, HOUSES.IDHOUSE AS IDHOUSE, HOUSENAME
FROM
`DEVICES`
JOIN `USERS` ON ( DEVICES.`IDUSER` = `USERS`.`IDUSER`)
LEFT JOIN `SERVICES` ON ( `DEVICES`.`IDDEVICE` = `SERVICES`.`IDDEVICE` )
LEFT JOIN ROOMS USING (IDROOM)
LEFT JOIN HOUSES USING (IDHOUSE)
ORDER BY `USERS`.`IDUSER` , `DEVICES`.`IDDEVICE`
  
  
  CREATE DEFINER=`alex`@`localhost` PROCEDURE `linkserviceroom`( IN ids INTEGER)
