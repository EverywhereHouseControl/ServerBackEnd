...
#define URL_FORMAT   "http://api.openweathermap.org/data/2.5/weather?q=%s&mode=json"
#define URL_SIZE   256


//we create struct Cclima  
struct coordinate{
	float lat;
	float lon;	
};
...
int main(int argc, char *argv[])
{
    ...
    if(argc != 2)
    {
        ...
        fprintf(stderr, " ejemplojson ciudad.\n\n");
        return 2;
    }
	...
	clima.cod=json_number_value(cod);
	
	//Here is the test done if everything is working
	printf("length %.2f\n",clima.coord.lon);
	printf("latitude %.2f\n",clima.coord.lat);
	printf("message %.2f\n",clima.sys.message);
	printf("country %s",clima.sys.country);
	printf("sunrise %d",clima.sys.sunrise);
	printf("sunset %d",clima.sys.sunset);
	printf("id %.2f",clima.weather.id);
	printf("main %s",clima.weather.main);	
	printf("description %s",clima.weather.description);
	printf("icon %s",clima.weather.icon);
	printf("base %s",clima.base);
	printf("temperature %.2f",clima.main.temp);
	printf("pressure %d",clima.main.pressure);
	printf("humidity %d",clima.main.humidity);
	printf("temp min %.2f",clima.main.temp_min);
	printf("temp max %.2f",clima.main.temp_max);
	printf("temp min %.2f",clima.wind.speed);
	printf("temp min %d",clima.wind.deg);
